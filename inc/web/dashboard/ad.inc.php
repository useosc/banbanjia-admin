...
if ($op == "post") {
    ...
    $id = intval($_GPC["id"]);
    if (0 < $id) {
        $slide = pdo_get("hello_banbanjia_slide", array("uniacid" => $_W["uniacid"], "id" => $id, "type" => "startpage"));
        if (empty($slide)) {
            imessage("引导页不存在或已删除", referer(), "error");
        }
    }
    if ($_W["ispost"]) {
    ...
    if ($_W["ispost"] && $_GPC["set"] == 1) {
        ...
        if (0 < $agentid) {
            foreach ($adid as $value) {
                pdo_update("hello_banbanjia_slide", array("agentid" => $agentid), array("uniacid" => $_W["uniacid"], "type" => "startpage", "id" => $value));
            }
        }
