...
function _login($forward = '')
{
    ...
    $record = array();
    $temp = pdo_get("hello_banbanjia_clerk", array("uniacid" => $_W["uniacid"], "mobile" => $username));
    if (!empty($temp)) {
        $password = md5(md5($temp["salt"] . $password) . $temp["salt"]);
        if ($password == $temp["password"]) {
            $record = $temp;
        }else{
            // imessage("登录失败，请检查您输入的用户名和密码！", "", "ajax");
        }
    }
    if(!empty($record)){
        if ($record["status"] == 2) {
        ...
        // if (empty($stores)) {
        //     imessage("您的申请是员工身份,没有权限管理公司！", "", "info");
        // }
        ...
        isetcookie("__hello_banbanjia_store", $session, 7 * 86400);
        if (empty($forward)) {
            $forward = $_GPC["forward"];
        }
        if (empty($forward)) {
            $forward = iurl("store/order/list");
        }
