...
';
    ...
    public function payResult($params) //支付回调
    {
        ...
        try {
            ...
            if ($params["result"] == "success" && $params["from"] == "notify" || $params["from"] == "return" && in_array($params["type"], array("carry", "move", "credit"))) {
                mload()->lmodel("order");
                ...
                if ($record['order_type'] == 'carry') {
                    ...
                    if (!empty($order) && !$order['is_pay']) {
                        ...
                        // if (!empty($params["prepay_id"])) { //请人代付
