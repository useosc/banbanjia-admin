...
class Events
{
    ...
    public static function onMessage($client_id, $message)
    {
        ...
        switch ($message['type']) {
            ...
            case 'changeGroup':
                ...
                // 记录该客服与该会员的服务结束
                self::$db->query("update `ims_hello_banbanjia_service_log` set `end_time` = " . time() . " where `client_id`= '" . $userClient . "'");
        ...
        foreach($kfList as $k=>$v){
            foreach($v as $key=>$vo){
                if(in_array($client_id, $vo['user_info'])){
                    ...
                    self::$db->query("update `ims_hello_banbanjia_service_log` set `end_time` = " . time() . " where `client_id`= '" . $client_id . "'");
    ...
    private static function userOnlineTask($client_id, $group)
    {
        ...
        if(1 == $res['code']){
            ...
            // 检测是否开启自动应答
            $sayHello = self::$db->query('select `word`,`status` from `ims_hello_banbanjia_service_reply` where `id` = 1');
    ...
    {
        $maxNumber = self::$db->query('select `max_service` from `ims_hello_banbanjia_service_config` where `id` = 1');
        if(!empty($maxNumber)){
            $maxNumber = 5;
        }else{
    ...
    private static function writeLog($flag = 1)
    {
        ...
        self::$db->update('ims_hello_banbanjia_service_now_data')->cols($param)->where('id=1')->query();
        ...
        if(2 == $flag){
            ...
            self::$db->insert('ims_hello_banbanjia_service_all_data')->cols($param)->query();
        }
