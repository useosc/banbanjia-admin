...
if ($op == 'list') {
    ...
    $agnetid = intval($_GPC['agentid']);
    if (0 < $agnetid) {
        $condition .= " and agentid = :agentid";
        $params['agentid'] = $agnetid;
    }
    ...
    if ($op == 'post') {
        ...
        if ($_W['ispost']) {
            ...
            if (!check_plugin_exist('diypage')) {
                imessage(error(-1, '注意：自定义功能目前仅对购买过“平台装修”插件的客户开放'), '', 'ajax');
            }
            if (!empty($id)) {
                pdo_update("hello_banbanjia_diypage", $diydata, array('id' => $id, 'uniacid' => $_W['uniacid']));
            } else {
                $diydata['addtime'] = TIMESTAMP;
                pdo_insert("hello_banbanjia_diypage", $diydata);
                $id = pdo_insertid();
            }
        ...
        if ($op == 'del') {
            ...
            foreach ($ids as $id) {
                pdo_delete("hello_banbanjia_diypage", array("uniacid" => $_W["uniacid"], "id" => $id));
            }
            ...
            if ($op == 'copy') {
                $id = intval($_GPC["id"]);
                $page = pdo_get("hello_banbanjia_diypage", array("uniacid" => $_W["uniacid"], "id" => $id));
                if (empty($page)) {
                    imessage(error(-1, "自定义页面不存在或已删除"), "", "ajax");
                }
                $page["name"] = $page["name"] . "-复制";
                unset($page["id"]);
                pdo_insert("hello_banbanjia_diypage", $page);
                    ...
                    if ($_W["ispost"] && $_GPC["set"] == 1) {
                        ...
                        if (0 < $agentid) {
                            foreach ($diypageid as $value) {
                                pdo_update("hello_banbanjia_diypage", array("agentid" => $agentid), array("uniacid" => $_W["uniacid"], "id" => $value));
                            }
                        }
