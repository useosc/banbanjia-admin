...
if ($op == 'post') {
    ...
    if ($_W['ispost']) {
        ...
        if (!empty($id)) {
            pdo_update("hello_banbanjia_diypage_menu", $menudata, array("id" => $id, 'uniacid' => $_GPC['uniacid']));
        } else {
            $menudata['addtime'] = TIMESTAMP;
            pdo_insert("hello_banbanjia_diypage_menu", $menudata);
            $id = pdo_insertid();
        }
    ...
    if ($op == 'del') {
        ...
        foreach ($ids as $id) {
            pdo_delete("hello_banbanjia_diypage_menu", array("uniacid" => $_GPC['uniacid'], "id" => $id));
        }
