...
if($op == 'index'){
    ...
    $members = pdo_fetchall("select * from " . tablename("hello_banbanjia_members") . $condition . " LIMIT " . ($pindex - 1) * $psize . "," . $psize, $params);
    foreach ($members as &$val) {
        $level1 = pdo_fetchcolumn("select count(*) from " . tablename("hello_banbanjia_members") . " where uniacid = :uniacid and spread1 = :spread1",array(":uniacid" => $_W['uniacid'],":spread1" => $val["uid"]));
        $level2 = pdo_fetchcolumn("select count(*) from " . tablename("hello_banbanjia_members") . " where uniacid = :uniacid and spread2 = :spread2", array(":uniacid" => $_W["uniacid"], ":spread2" => $val["uid"]));
