...
if($op == 'list'){
    ...
    $psize = 15;
    $total = pdo_fetchcolumn("select count(*) FROM " . tablename("hello_banbanjia_superredpacket") . $condition, $params);
    $superRedpackets = pdo_fetchall("select * from " . tablename("hello_banbanjia_superredpacket") . $condition . " order by id desc limit " . ($pindex - 1) * $psize . "," . $psize, $params);
...
if ($op == "post") {
    ...
    if ($_W["ispost"]) {
        ...
        if ($data["customer"]["type"] == 1) {
            ...
            $uid = pdo_fetchall("select uid from " . tablename("hello_banbanjia_members") . $condition, $params, "uid");
            $uid = array_keys($uid);
        }
        if (empty($uid)) {
        ...
        if (!empty($id)) {
            pdo_update("hello_banbanjia_superredpacket", $menudata, array("id" => $id, "uniacid" => $_W["uniacid"]));
        } else {
            $menudata["addtime"] = TIMESTAMP;
            pdo_insert("hello_banbanjia_superredpacket", $menudata);
            $id = pdo_insertid();
        }
        imessage(error(0, "超级红包设置成功"), iurl("superRedpacket/grant/send", array("id" => $id)), "ajax");
    }
    if (!empty($id)) {
        $superRedpacket = pdo_fetch("select * from " . tablename("hello_banbanjia_superredpacket") . " where id = :id and uniacid = :uniacid", array(":id" => $id, ":uniacid" => $_W["uniacid"]));
...
if ($op == "del") {
    ...
    foreach ($ids as $id) {
        pdo_delete("hello_banbanjia_superredpacket", array("uniacid" => $_W["uniacid"], "id" => $id));
    }
...
if ($op == "send") {
    ...
    if ($_W["ispost"]) {
        ...
        foreach ($uids as $key => $uid) {
            ...
            if ($superRedpacket["data"]["customer"]["template_notice"] == 1) {
                ...
                if (!empty($openid)) {
                    ...
                    $status = $acc->sendTplNotice($openid, $_W["we7_hello_banbanjia"]["config"]["notice"]["wechat"]["account_change_tpl"], $send, $url);
                    if (is_error($status)) {
        ...
        $superRedpacket["grant_object"]["unissued_uid"] = array_values($superRedpacket["grant_object"]["unissued_uid"]);
        pdo_update("hello_banbanjia_superredpacket", array("grant_object" => iserializer($superRedpacket["grant_object"])), array("id" => $id, "uniacid" => $_W["uniacid"]));
