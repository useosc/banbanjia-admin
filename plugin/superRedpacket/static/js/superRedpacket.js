define(['jquery.ui','clockpicker'],function(ui,$){
    ...
    superRedpacket.init = function(params){
        ...
        if(!superRedpacket.data) {
            superRedpacket.data = {
                name: '超级红包',
                page: {
                    image: '../addons/hello_banbanjia/plugin/superRedpacket/static/img/header.png',
                    text: {
						color: '#fb584f',
    ...
    superRedpacket.tplEditor = function() {
		...
		$("#app-editor .category-add").unbind('click').click(function() {
			...
			if(superRedpacket.data.redpackets[itemid].scene == 'carry') {
    ...
    superRedpacket.save = function() {
		$(".btn-save").unbind('click').click(function() {
			...
			Notify.confirm('确定保存并开始发放吗?', function() {
				...
				var posturl = "./index.php?c=site&a=entry&ctrl=superRedpacket&ac=grant&op=post&do=web&m=hello_banbanjia";
