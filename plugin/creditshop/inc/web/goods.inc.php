...
if ($op == "list") {
    ...
    if ($_W["ispost"]) {
        if (!empty($_GPC["ids"])) {
            foreach ($_GPC["ids"] as $k => $v) {
                ...
                pdo_update("hello_banbanjia_creditshop_goods", $data, array("uniacid" => $_W["uniacid"], "id" => intval($v)));
            }
        }
    ...
    $lists = pdo_fetchall("select * from" . tablename("hello_banbanjia_creditshop_goods") . $condition . " order by displayorder desc", $params);
}
if ($op == "post") {
    $_W["page"]["title"] = "编辑商品";
    $id = intval($_GPC["id"]);
    ...
    if ($_W["ispost"]) {
        ...
        if (0 < $id) {
            pdo_update("hello_banbanjia_creditshop_goods", $data, array("uniacid" => $_W["uniacid"], "id" => $id));
        } else {
            pdo_insert("hello_banbanjia_creditshop_goods", $data);
        }
...
if ($op == "status") {
    ...
    $state = pdo_update("hello_banbanjia_creditshop_goods", array("status" => $status), array("uniacid" => $_W["uniacid"], "id" => $id));
    if ($state === false) {
...
if ($op == "del") {
    ...
    foreach ($ids as $v) {
        pdo_delete("hello_banbanjia_creditshop_goods", array("uniacid" => $_W["uniacid"], "id" => $v));
    }
